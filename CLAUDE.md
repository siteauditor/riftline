...
**The controls are Radix primitives** under `src/components/ui/` (they began as shadcn/ui's and are restyled in the site's language; edit them freely, they are ours). Use `SelectField` for every select (Radix refuses an empty item value, so an "any" choice is a word like `all` mapped back to null), `Hint` (a Tooltip; its child must be focusable, so wrap a disabled button in a `span tabIndex={0}`) rather than a `title` attribute, `Button` or `buttonVariants` rather than a hand-styled button, `ChipGroup` and `Chip` for a row of filter pills, `Tabs` for in-page tabs and `NavTabs` for tabs that are routes (both slide one gold bar between items through `src/lib/useSlidingBar.ts`; a trigger's own edge is hidden while the bar is present, `OWN_EDGE_CLASS`). Confirm a completed action with a toast (`toast()` from sonner; the `Toaster` in `App.tsx` carries the site's colours) and a figure that may count up with `CountUp` (it counts only on a page the app navigated to, since a prerendered figure must hydrate at its value). `@/` is `src/`. The site is dark only: `<html class="dark">` makes the components' `dark:` variants the only ones. A control that shows a value must render it on the server too (see `SelectField`, which hands the label to `SelectValue`); Radix fills some things in after mount. Anything that drops over the page (a suggestion list, a menu) is portalled through a Radix `Popover` or `Select`, never positioned in place: the art headers hide their overflow and clip it.
